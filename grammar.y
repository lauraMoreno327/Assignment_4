...
construct_repeat: 
    ...
    T_RPAR
    {
      ...
      int jump_dst = @2.begin.line; // not sure about this one
...
assignment : T_ID arr_index T_ASSIGN a_expr 
    {
      ...
      if (sym->datatype != src_temp->datatype)
      {
        ...
        if (sym->datatype == DTYPE_INT)
          itab_instruction_add (itab, OP_CAST_FLOAT2INT, sym->datatype, UNUSED_ARG, src_temp->datatype);
        else
          itab_instruction_add (itab, OP_CAST_INT2FLOAT, sym->datatype, UNUSED_ARG,src_temp->datatype );
